...
\item Fotografie: 321 fotografií v rozlišení $4094\times4096$ px
\end{itemize}

\subsection{Model 2}
Jde o rekonstrukci sochy.
\begin{itemize}
\item 3D model: 704 940 trojúhelníků a 2 114 820 vrcholů.
\item Fotografie: 37 fotografií v rozlišení $2400\times3200$ px
\end{itemize}
...
\caption{Graf zobrazující čas renderování snímku při použití různého množství vstupních fotografií.}
\label{fig:data-1}
\end{center}
\end{figure}
...
Pro potvrzení předchozího tvrzení, že výrazné výchylky způsobuje načítání fotografií s velkým rozlišením do GPU jsem provedl měření, při kterém bylo načítání originálních velikostí vypnuté. V tomto měření se žádné výkyvy neprojevily, žádný snímek nepřekročil 11ms. Výsledky měření jsou zobrazeny v grafu \ref{fig:data-2}
...
V grafu je vidět, že při zapnutí obou průchodů je rendering výrazně pomalejší, průměrný čas pro vyrenderování snímku je 65,41ms, to odpovídá 15,3 fps. V tomto případě již nemůže mluvit o real-time renderingu, jde spíše o interaktivních fps.  Oproti tomu samotný texturovací průchod má průměrnou dobu vykreslení snímku 5,60ms (176 fps). Dále je při obou zapnutých  průchodech vidět výrazný nárůst vrcholů způsobených načítáním textur do GPU. To je dané tím, že při pomalejším renderingu snímků se stíhají nahrát plné velikosti všech fotografií, takže je jejich přenos do GPU mnohem častější.

\subsubsection*{Model 2}

První testování proběhlo stejně jako v předchozím případě bez zapnutého průchodu pro doplňování neotexturovaných ploch. Tento model má poměrně jednoduchou geometrii a tento průchod pro něj nepřináší vylepšené texturování, proto nebyl při testování vůbec použit. 
...
V grafu je zobrazeno podobné chování, jako u předchozího modelu. Při načítání fotografií je znatelné zpomalení vykreslování, ale protože tento model má menší rozlišení fotografií. maximální časy nejsou tak výrazné. Celkově je renderování pomalejší, ale to je způsobeno zejména výrazně složitějším modelem, který má přes 700 tisíc trojúhelníků. Data jsou opět shrnuta v tabulce \ref{tab:test-4}.
